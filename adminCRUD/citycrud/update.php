...
if($imageFileType != null){
	if ($imageFileType != "jpg" ) {
		echo "<script>alert('Sorry, only JPG files are allowed. " . $imageFileType . "'); history.go(-1);</script>";
		$uploadOk = 0;
		exit();
	}
}
...
if ($uploadOk == 0) {
	...
	if($imageFileType != null){
		if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
			...
			$stmt->bind_param("iss", $city_id, $type_id, $filename);
			$stmt->execute();
			echo "<script>alert('Successfully update data.'); location.href = 'city.php';</script>";
			exit();  
		}else {
